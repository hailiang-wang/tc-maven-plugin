 ...
 *
 * @see http://forge.terracotta.org/projects/tc-maven-plugin/surefire.html
 */
public class DsoSurefireMojo extends DsoLifecycleMojo {
  ...
  private static final String SUREFIRE_MODULE_NAME = "clustered-surefire-2.3";
  ...
  protected void onExecute() throws MojoExecutionException, MojoFailureException {
    ...
    if (artifact == null) {
      throw new MojoExecutionException("Unable to find terracotta artifact");
    }

    surefireModule = Module.Factory.newInstance();
    surefireModule.setGroupId(SUREFIRE_MODULE_GROUP_ID);
    surefireModule.setName(SUREFIRE_MODULE_NAME);
    surefireModule.setVersion(artifact.getVersion());
  ...
  class SurefireThread implements Runnable {
    ...
    private void runSurefire() {
      ...
      try {
        ...
        boolean success = (surefireBooter.run() == 0);
  ...
  private SurefireBooter constructSurefireBooter(File reportsDirectory, String nodeName) throws MojoExecutionException,
    ...
    if (fork.isForking()) {
      ...
      args += " -Dtc.session.classpath=" + quoteIfNeeded(createSessionClasspath());

      String modulesRepository = getModulesRepository();
      if(modulesRepository.indexOf(' ')>-1) {
        args += " \"-Dcom.tc.l1.modules.repositories=" + modulesRepository+"\"";
      } else {
        args += " -Dcom.tc.l1.modules.repositories=" + modulesRepository;
      }
      args += " -Dcom.tc.l1.modules.additional=" + surefireModule.getGroupId() + '.' + surefireModule.getName()
          + "\\;bundle-version:=" + surefireModule.getVersion().replaceAll("-SNAPSHOT", ".SNAPSHOT");

      if (argLine != null && argLine.trim().length() > 0) {
        args += " " + argLine;
      }

      if (config != null) args += " -Dtc.config=" + config.getAbsolutePath();
    ...
    surefireBooter.setEnableAssertions(true);
  ...
  private void addProvider(SurefireBooter surefireBooter, String provider, String version, Artifact filteredArtifact)
    ...
    ArtifactResolutionResult result = resolver.resolveArtifact(filteredArtifact, providerArtifact, localRepository, remoteRepositories);
  ...
  private void addArtifact(SurefireBooter surefireBooter, Artifact surefireArtifact) throws ArtifactNotFoundException,
    ...
    ArtifactResolutionResult result = resolver.resolveArtifact(null, surefireArtifact, localRepository, remoteRepositories);
